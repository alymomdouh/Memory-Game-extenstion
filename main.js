...
var bolcknumber = 8; // number of imgs in game
var yourName = "";
var Score;
var score = 0;
var time = 120;
...
window.onload = function () {
  ...
  /////////////////////////////////////////
  if(mylevel){
    chrome.storage.sync.get(["mylevel"],function(vvv){
        bolcknumber=parseInt(vvv.mylevel);
        console.log("inside chrome.storage.sync "+parseInt(vvv.mylevel));
    }); 
};
// function makechangeingame(value) {
//    = parseInt(value);
//   console.log("afterchange in function " + bolcknumber);
// }
...
function makeclickongamelevel() {
  ...
  document
    .querySelector(".control-buttons .span .test .high")
    .addEventListener("click", function () {
...
function makeblock(level) {
  ...
  console.log("the bolcknumber is" + bolcknumber);
  var bn = bolcknumber / 2;
  if (level == "easy" && bn == 4) {
    removeelements(x);
    for (let i = 0; i < 4; i++) {
      //makeelement(i);
...
function makeelement(index) {
  let imgname = imagessource[index].substring(
    0,
    imagessource[index].length - 4
  );
  //console.log(imgname);
...
function stopClicking() {
  ...
  var blocksContainer =
    document.getElementsByClassName("memory-game-blocks")[0];
...
function checkMatchedBlocks(firstBlock, secondBlock) {
  ...
  if (
    firstBlock.getAttribute("imagename") ===
    secondBlock.getAttribute("imagename")
  ) {
...
function gamefinshedornot() {
  ...
  if (allCardsIsMatched.length === bolcknumber) {
    ...
    if (yourName == null || yourName == "") {
      document.querySelector(".modal-content p").innerHTML =
        "Congrats, Anonym 👏,Your SCore : " +
        score +
        "  You Won The Game But After " +
        numberOfWrongTries +
